...
<div class="grid grid-cols-1 md:grid-cols-2 gap-4">
    <div>
        ...
        <input type="text" name="code" value="{{ old('code', $gearItemCategory->code ?? '') }}"
            class="w-full border-gray-300 rounded-md">
        <p class="text-xs text-gray-500 mt-1">
            例）BOARD, BOOTS など（未使用なら空でも可）
        </p>
        @error('code')
    ...
    <div>
        ...
        <input type="text" name="name" value="{{ old('name', $gearItemCategory->name ?? '') }}"
            class="w-full border-gray-300 rounded-md">
        <p class="text-xs text-gray-500 mt-1">
            例）ボード、ブーツ、ウェア など
        </p>
    ...
    <div>
        ...
        <input type="number" name="sort_order" min="0"
            value="{{ old('sort_order', $gearItemCategory->sort_order ?? 0) }}"
    ...
    <div>
        ...
        <select name="is_active" class="w-full border-gray-300 rounded-md">
            <option value="1" @selected(old('is_active', $gearItemCategory->is_active ?? 1) == 1)>有効</option>
            <option value="0" @selected(old('is_active', $gearItemCategory->is_active ?? 1) == 0)>無効</option>
        </select>
        @error('is_active')
            <p class="mt-1 text-xs text-red-600">{{ $message }}</p>
        @enderror
